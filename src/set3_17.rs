...
mod test {
    ...
    // deployed serverside in web applications; the second function
    // models the server's consumption of an encrypted session token,
    ...
    // decryption function. The leak is the error message that the padding
    // is valid or not.
    //
    ...
    impl Oracle {
        fn new() -> Oracle {
            ...
            let idx = rng.gen::<usize>() % 10;
            let x = strings.iter().nth(idx).unwrap();
            
            Oracle {
                key: random_key(16),
                data: x.from_base64().unwrap()
            }
        }
